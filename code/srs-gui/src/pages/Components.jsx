...
function ComponentsImpl() {
    ...
    const handleUpgradeStrategyChange = React.useCallback((e) => {
        ...
        axios.post(addURLPrefix(MGMT_HOST, MGMT_PORT, '/terraform/v1/mgmt/strategy'), {
            ...token,
        }).then(res => {
    ...
    React.useEffect(() => {
        const token = Token.load();
        axios.post(addURLPrefix(MGMT_HOST, MGMT_PORT, '/terraform/v1/mgmt/containers'), {
            ...token, action: 'query',
        }).then(res => {
...
function MgmtUpgradeButton({ onStatus }) {
    ...
    React.useEffect(() => {
        const refreshMgmtStatus = () => {
            const token = Token.load();
            axios.post(addURLPrefix(MGMT_HOST, MGMT_PORT, '/terraform/v1/mgmt/status'), {
                ...token,
            }).then(res => {
    ...
    const handleStartUpgrade = () => {
        ...
        const token = Token.load();
        axios.post(addURLPrefix(MGMT_HOST, MGMT_PORT, '/terraform/v1/mgmt/upgrade'), {
            ...token,
        }).then(res => {
