...
function ScenarioForwardCn() {
    ...
    React.useEffect(() => {
        ...
        axios.post(addURLPrefix(MGMT_HOST, MGMT_PORT, '/terraform/v1/ffmpeg/forward/secret'), {
            ...token,
        }).then(res => {
...
function ScenarioForwardImpl({ defaultActiveKey, defaultSecrets }) {
    ...
    React.useEffect(() => {
        const refreshStreams = () => {
            const token = Token.load();
            axios.post(addURLPrefix(MGMT_HOST, MGMT_PORT, '/terraform/v1/ffmpeg/forward/streams'), {
                ...token,
            }).then(res => {
