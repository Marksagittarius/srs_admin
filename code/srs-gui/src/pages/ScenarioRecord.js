...
function ScenarioRecordImpl({ activeKey, defaultApplyAll, recordHome }) {
    ...
    React.useEffect(() => {
        const refreshRecordFiles = () => {
            ...
            axios.post(addURLPrefix(MGMT_HOST, MGMT_PORT, '/terraform/v1/hooks/record/files'), {
                ...token,
            }).then(res => {
    ...
    const setupRecordPattern = (e) => {
        ...
        const token = Token.load();
        axios.post(addURLPrefix(MGMT_HOST, MGMT_PORT, '/terraform/v1/hooks/record/apply'), {
            ...token, all: !!recordAll,
        }).then(res => {
            alert('设置录制规则成功');
    ...
    const removeRecord = React.useCallback((file) => {
        const token = Token.load();
        axios.post(addURLPrefix(MGMT_HOST, MGMT_PORT, '/terraform/v1/hooks/record/remove'), {
