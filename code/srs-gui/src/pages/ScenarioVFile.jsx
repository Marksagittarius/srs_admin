...
function ScenarioVFileCn() {
    ...
    React.useEffect(() => {
        ...
        axios.post(addURLPrefix(MGMT_HOST, MGMT_PORT, '/terraform/v1/ffmpeg/vlive/secret'), {
            ...token,
        }).then(res => {
...
function ScenarioVFileImpl({ defaultActiveKey, defaultSecrets }) {
    ...
    React.useEffect(() => {
        const refreshStreams = () => {
            const token = Token.load();
            axios.post(addURLPrefix(MGMT_HOST, MGMT_PORT, '/terraform/v1/ffmpeg/vlive/streams'), {
                ...token,
            }).then(res => {
...
function VLiveFileUploader({ platform, vLiveFiles, setVLiveFiles }) {
    ...
    const updateSources = React.useCallback((platform, files, setFiles) => {
        ...
        const token = Token.load();
        axios.post(addURLPrefix(MGMT_HOST, MGMT_PORT, '/terraform/v1/ffmpeg/vlive/source'), {
